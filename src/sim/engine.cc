...
void Engine::getState(i32 podId, f32* state) {
  ...
  *(state++) = pod.boosted;
  ...
  Vector podFace{std::cos(pod.angle), std::sin(pod.angle)};
  *(state++) = podFace.x;
  *(state++) = podFace.y;

  Vector podV{pod.vx, pod.vy};
  i32 rv = std::hypot(pod.vx, pod.vy);
  *(state++) = pod.vx / rv;
  *(state++) = pod.vy / rv;
  *(state++) = rv;

  i32 finishIndex = laps_ * static_cast<i32>(track_.size());
  *(state++) = (f32) pod.next / finishIndex;

  for (i32 diffId : {1, 2, 3}) {
    i32 otherId = podId ^ diffId;
    Pod const& other = pods_[otherId];
    Vector otherV{other.vx, other.vy};

    i32 dx = other.x - pod.x;
    i32 dy = other.y - pod.y;
    i32 r = std::hypot(dx, dy);
    *(state++) = (f32) (other.next - pod.next) / finishIndex;
    *(state++) = dx / r;
    *(state++) = dy / r;
    *(state++) = r / POD_RADIUS;
    *(state++) = dot(podV, otherV);
    *(state++) = cross(podV, otherV);

    Vector otherFace{std::cos(other.angle), std::sin(other.angle)};
    *(state++) = dot(podFace, otherFace);
    *(state++) = cross(podFase, otherFace);
  }

  for (i32 cpId = pod.next; cpId < pod.next + 4; cpId++) {
    Checkpoint cp = track_[cpId % track_.size()];

    Vector diff{cp.x - pod.x, cp.y - pod.y};

    *(state++) = std::hypot(diff.x, diff.y);
    *(state++) = dot(podV, diff);
    *(state++) = cross(podV, diff);
    *(state++) = dot(podFace, diff);
    *(state++) = cross(podFace, diff);

    Checkpoint cpNext = track_[(cpId + 1) % track_.size()];
    Vector diffNext{cpNext.x - cp.x, cpNext.y - cp.y};

    *(state++) = std::hypot(diffNext.x, diffNext.y);
    *(state++) = dot(podV, diffNext);
    *(state++) = cross(podV, diffNext);

    *(state++) = dot(podFace, diffNext);
    *(state++) = cross(podFace, diffNext);
  }
}
