...
struct Divings {
    ...
    uint8_t score[PLAYER_COUNT];
    int8_t combo[PLAYER_COUNT];

    bool end;

    inline bool is_useless([[maybe_unused]] int8_t player_idx) const {
        return false;
    }

    inline uint8_t greedy_moves([[maybe_unused]] int8_t player_idx) const {
        return (goal & 3);
    }
