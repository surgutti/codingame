...
def analyze_games(games):
	...
	for player_id, game_id in games:
		...
		for frame in replay['frames']:
			...
			if 'summary' in frame:
				...
				if 'Hurdle' in summary:
					hurdle_games += 1

				if 'Archery' in summary:
					archery_games += 1

				if 'Skating' in summary:
					skating_games += 1

				if 'Diving' in summary:
					diving_games += 1
	...
	archery_games /= len(games)
	skating_games /= len(games)
	diving_games /= len(games)
