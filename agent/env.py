...
class VecEnv:
  def __init__(self, num_envs, seed=42, device="cpu"):
    ...
    self.state_cpu = torch.zeros((num_envs, RAW_STATE_DIM), dtype=torch.float32)
    self.rewards_cpu = torch.zeros((num_envs,), dtype=torch.float32)
    self.dones_cpu = torch.zeros((num_envs,), dtype=torch.float32)
