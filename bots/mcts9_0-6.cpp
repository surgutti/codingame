// *** Start of: /home/olaf/codingame/main.cpp *** 
 ...
 struct State {
     ...
     void get_stats(float& r0, float& r1, float& r2) {
         ...
         if (totaly_losing[0]) {
             int losing_sum = score0 + std::min(score1, score2);
             r0 = (float) (score0 - std::min(score1, score2)) / losing_sum;
         }
         else {
         ...
         if (totaly_losing[1]) {
             int losing_sum = score1 + std::min(score0, score2);
             r1 = (float) (score1 - std::min(score0, score2)) / losing_sum;
         }
         else {
         ...
         if (totaly_losing[2]) {
             int losing_sum = score2 + std::min(score0, score1);
             r2 = (float) (score2 - std::min(score0, score1)) / losing_sum;
         }
         else {
     ...
     void init(const std::vector<std::string>& gpu,
         ...
         totaly_losing[0] = false;
         totaly_losing[1] = false;
         totaly_losing[2] = false;
...
int main() {
    ...
    for (TURN = 0; ; TURN++) {
        ...
        if (final_score[0] > 0 && final_score[1] > 0 && final_score[2] > 0) {
            if (final_score[0] < final_score[1] && final_score[0] < final_score[2]) {
                current_state.totaly_losing[0] = true;
            }

            if (final_score[1] < final_score[0] && final_score[1] < final_score[2]) {
                current_state.totaly_losing[1] = true;
            }

            if (final_score[2] < final_score[0] && final_score[2] < final_score[1]) {
                current_state.totaly_losing[2] = true;
            }
        }
