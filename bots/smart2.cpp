// *** Start of: /home/olaf/codingame/brain.cpp *** 
 ...
 #define STATE_HPP
  ...
  // *** Start of: /home/olaf/codingame/minigames/hurdles.hpp *** 
  #ifndef HURDLES
  #define HURDLES
  ...
  #endif // HURDLES
  // *** End of: /home/olaf/codingame/minigames/hurdles.hpp *** 
  ...
  // *** Start of: /home/olaf/codingame/minigames/skating.hpp *** 
  #ifndef SKATING
  #define SKATING
  ...
  #endif // SKATING
  // *** End of: /home/olaf/codingame/minigames/skating.hpp *** 
  // *** Start of: /home/olaf/codingame/minigames/divings.hpp *** 
  #ifndef DIVINGS_HPP
  #define DIVINGS_HPP
  ...
  #endif // DIVINGS_HPP
  // *** End of: /home/olaf/codingame/minigames/divings.hpp *** 
 ...
 struct State {
     ...
     int8_t greedy_move(int player_idx) const {
         float sum = hurdles_score[player_idx] +
                     archery_score[player_idx] +
                     skating_score[player_idx] +
                     divings_score[player_idx] + 1;
                     
         float weights[4] = {0.31946663887537924, 0.18851756640959727, 0.2367701892123486, 0.2552456055026749}; // default random weights
 
         if (!hurdles.is_useless(player_idx)) {
             const float weight = 5 - 5 * hurdles_score[player_idx] / sum;
         ...
         if (!archery.is_useless(player_idx)) {
             const float weight = 5 - 5 * archery_score[player_idx] / sum;
         ...
         if (!skating.is_useless(player_idx)) {
             const float weight = 5 - 5 * skating_score[player_idx] / sum;
         ...
         if (!divings.is_useless(player_idx)) {
             const float weight = 5 - 5 * divings_score[player_idx] / sum;
 ...
 // *** Start of: /home/olaf/codingame/brain.hpp *** 
 #ifndef BRAIN_HPP
 #define BRAIN_HPP
     ...
     inline void optimize(BrainNode** heads, State &state, float* reward) {
         ...
         for (int i = 0; i < 3; i++) {
             if (heads[i] == 0 || heads[i]->vis == 0) {
                 childs[i] = 0;
                 moves[i] = state.greedy_move(i); // random_move();
             }
             else {
                 if (heads[i]->sons == 0) {
                     heads[i]->expand();
 ...
 // *** End of: /home/olaf/codingame/brain.hpp *** 
