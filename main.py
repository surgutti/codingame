...
def collision(dx1, dy1, dx2, dy2, ux3, uy3):
    if distance(dx1, dy1, ux3, uy3) <= 505:
        return True

    ugly_vx = dx1 - ux3
    ugly_vy = dy1 - uy3

    if dx1 == dx2 and dy1 == dy2 and ugly_vx == 0 and ugly_vy == 0:
        return False
...
while True:
    ...
    for i in range(my_drone_count):
        ...
        for alpha in range(360):
            angle = alpha * 2 * math.pi / 360
            xx = drone_x + math.cos(angle) * 600
            yy = drone_y + math.sin(angle) * 600
            ...
            for ugly_x, ugly_y, vx, vy in ugly:
                if collision(drone_x, drone_y, xx, yy, ugly_x, ugly_y):
                    ok = False
