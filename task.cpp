...
INLINE uint32_t board_hash(Board const& board) {
    __m128i b = _mm_set1_epi32(board);

    __m128i masks = _mm_set_epi32(0,
                                  0b111000000111000000111000000,
                                  0b000111000000111000000111000,
                                  0b000000111000000111000000111);

    __m128i shifts = _mm_set_epi32(0, 6, 3, 0);
    __m128i extracted = _mm_and_si128(b, masks);
    extracted = _mm_srlv_epi32(extracted, shifts);

    __m128i low3  = _mm_and_si128(extracted, _mm_set1_epi32(0b111));
    __m128i mid3  = _mm_and_si128(_mm_srli_epi32(extracted, 9), _mm_set1_epi32(0b111));
    __m128i high3 = _mm_srli_epi32(extracted, 18);

    __m128i val = _mm_add_epi32(
                    _mm_add_epi32(
                        _mm_mullo_epi32(low3,  _mm_set1_epi32(1000000)),
                        _mm_mullo_epi32(mid3,  _mm_set1_epi32(1000))),
                    high3);

    __m128i weights = _mm_set_epi32(0, 1, 10, 100);
    __m128i p = _mm_mullo_epi32(val, weights);
...
uint32_t ans = 0;
alignas(64) State beam[2][MAX_STATES];
int len[2];
...
// /*
constexpr uint32_t lookup90[512] = {
...
INLINE Board rot(Board const& board) {
	/*
	return ((board & 0b000000000111000000000000111) <<  6) |
		   ((board & 0b000000000000000000000111000) << 12) |
		   ((board & 0b000000000000000000111000000) << 18) |
		   ((board & 0b111000000000000111000000000) >>  6) |
		   ((board & 0b000000000000111000000000000)      ) |
		   ((board & 0b000000111000000000000000000) >> 18) |
		   ((board & 0b000111000000000000000000000) >> 12);	
	*/
	// /*
	return ((lookup90[(board      ) & 0b111111111]) << 6) |
		   ((lookup90[(board >>  9) & 0b111111111]) << 3) |
		   ((lookup90[(board >> 18) & 0b111111111])     );
	// */
}
...
INLINE void generate_moves(int const nxt, Board const& board, uint32_t const *ile) {
	...
	*/
	...
	uint32_t empty = ((board & 0b001001001001001001001001001) |
					 ((board & 0b010010010010010010010010010) >> 1) |
					 ((board & 0b100100100100100100100100100) >> 2))
						     ^ 0b001001001001001001001001001;		
