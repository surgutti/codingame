...
int main() {
	...
	for (int d = 0; d < depth; d++) {
		...
		for (int j = 0; j < length; j++) {
			...
			// cerr << "0: " << board_hash(board) << '\n';
			board = rot(board); UPD(1)
			// cerr << "1: " << board_hash(board) << '\n';
			board = rot(board); UPD(2)
			// cerr << "2: " << board_hash(board) << '\n';
			board = rot(board); UPD(3)
			// cerr << "3: " << board_hash(board) << '\n';
			board = sym(board); UPD(4)
			// cerr << "4: " << board_hash(board) << '\n';
			board = rot(board); UPD(5)
			// cerr << "5: " << board_hash(board) << '\n';
			board = rot(board); UPD(6)
			// cerr << "6: " << board_hash(board) << '\n';
			board = rot(board); UPD(7)
			// cerr << "7: " << board_hash(board) << '\n';
